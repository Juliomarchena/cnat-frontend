 ...
 *
 * v3.2 — Formato tabla clara, todas las fuentes, ordenado por proximidad
 */
...
function buildPrompt(data) {
  ...
  eqs.filter(e => e.latitude && e.longitude && e.magnitude >= 4.0).forEach(e => {
    ...
    const lugar = (e.place||'').substring(0,25).toLowerCase().replace(/\s+/g,'_');
    const key   = `${t}_${mag}_${lugar}`;
    if (!grupos[key]) grupos[key] = [];
    grupos[key].push(e);
  });

  // ── Calcular distancia a Lima y clasificar por proximidad ──
  const eventos = Object.values(grupos).map(grupo => {
    ...
    const lon  = parseFloat(rep.longitude);
    const R    = 6371;
    const dLat = ((lat - (-12.05)) * Math.PI) / 180; // Lima -12.05, -77.03
    ...
    const fuentes = grupo
      ...
      .map(e => `${(e.source_id||'?').toUpperCase()}:M${e.magnitude}`)
      .join(' | ');
    return { rep, dist, nivel, fecha, fuentes };
  }).sort((a,b) => a.dist - b.dist); // del más cercano al más lejano

  const cercanos = eventos.filter(e => e.nivel !== 'REMOTO').slice(0, 8);
  const remotos  = eventos.filter(e => e.nivel === 'REMOTO' && e.rep.magnitude >= 5.5).slice(0, 3);

  const lista = cercanos.length > 0
    ? cercanos.map(e =>
        `${e.fecha} | ${e.rep.place||'Ubicación desconocida'} | ~${e.dist}km de Lima | Prof:${e.rep.depth_km}km\n   ${e.fuentes}`
      ).join('\n')
    : 'Sin eventos cercanos al Perú en este momento';

  return `Eres ARIA del CNAT (Centro Nacional de Alerta de Tsunamis). 
Genera el reporte EXACTAMENTE en este formato — sin cambiar nada:

🟢/🟡/🔴 ESTADO: [una frase sobre situación general] — ${now}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ÚLTIMOS SISMOS DETECTADOS CERCANOS AL PERÚ:
(del más cercano al más lejano de Lima)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[Aquí van los sismos, cada uno en 2 líneas exactas así:]
DD/MM HH:MM | LUGAR | ~XXXkm de Lima | Prof:XXkm
...
RECOMENDACIÓN: [una línea operativa concreta]

═══════════════════════════════════════════
DATOS REALES (ya ordenados del más cercano al más lejano de Lima):
${lista}
${remotos.length>0 ? `\nRemotos M5.5+: ${remotos.map(e=>`M${e.rep.magnitude} ${e.rep.place}`).join(' | ')}` : ''}
═══════════════════════════════════════════

REGLAS CRÍTICAS — NO NEGOCIABLES:
1. Copia EXACTAMENTE el formato de 2 líneas por sismo
2. En la línea 2 de cada sismo, pon TODAS las fuentes con sus magnitudes separadas por |
   Ejemplo: USGS:M5.8 | IRIS:M6.5 | IGP:M6.9
3. Si un sismo tiene 3 fuentes con magnitudes diferentes, muéstralas TODAS — no elijas una
4. Ordena siempre del más cercano al más lejano de Lima
5. Sin markdown, sin asteriscos, sin texto adicional fuera del formato`;
}
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        ...
        {/* ── Header con REFRESH siempre visible arriba ── */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {/* ── Botón REFRESH — siempre visible arriba ── */}
            <button
        ...
        <div style={{ borderTop: '1px solid #00ff0022', paddingTop: 5 }}>
          <div style={{
            fontSize: 11, color: '#00ff00', lineHeight: 1.5,
        ...
        <div style={{
          ...
          <span style={{ fontSize: 8, color: '#00ff0044', fontFamily: "'Courier New', monospace" }}>CNAT::ARIA::v3.2 | Auto:5min</span>
