...
export default function App() {
  ...
  const pingRef = useRef(null); const focusTimer = useRef(null);
  const playAlert5s = useCallback(() => { try { if(!pingRef.current) pingRef.current=new(window.AudioContext||window.webkitAudioContext)(); const ctx=pingRef.current; const beep=(t)=>{ const o=ctx.createOscillator(),g=ctx.createGain(); o.connect(g);g.connect(ctx.destination); o.type='sine'; o.frequency.setValueAtTime(1400,t); o.frequency.exponentialRampToValueAtTime(900,t+0.2); g.gain.setValueAtTime(0.5,t); g.gain.exponentialRampToValueAtTime(0.001,t+0.4); o.start(t); o.stop(t+0.4); }; const now=ctx.currentTime; [0,0.6,1.2,1.8,2.4,3.0,3.6,4.2,4.8].forEach(dt=>beep(now+dt)); }catch(e){} }, []);
  const focusEq = useCallback((id) => { setFocusedEqId(id); setTab('mapa'); playAlert5s(); if(focusTimer.current) clearTimeout(focusTimer.current); focusTimer.current=setTimeout(()=>setFocusedEqId(null), 5000); }, [playAlert5s]);
    ...
    <div style={{ background: '#050b18', color: '#e2e8f0', minHeight: '100vh', fontFamily: "'JetBrains Mono',monospace" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: tab === 'mareografo' ? '1fr' : '1fr 380px', gap: 0, height: 'calc(100vh - 220px)' }}>
        <div style={{ padding: tab === 'mareografo' ? 0 : 12, overflow: 'auto', height: '100%' }}>
          {tab === 'mapa' && <div style={{ display: 'flex', flexDirection: 'column', gap: 0, height: '100%' }}><div style={{ flex: 1, borderRadius: 10, overflow: 'hidden', border: '2px solid #1e3a5f', minHeight: 280 }}><PacificMap earthquakes={eq} buoys={bu} focusedEqId={focusedEqId} onClearFocus={clearFocus} /></div><MapLegend /><div style={{ height: 220 }}><AnalyticsDashboard earthquakes={eq} buoys={bu} sources={sr} data={data} /></div></div>}
