...
function TideGaugeMap() {
  ...
  const mapStationFields = (s) => {
    ...
    return { code: String(code).trim(), name: String(name).trim(), country: String(country).trim(), lat, lon, status, sensor_type: s.sensor_type || s.sensor || '', performance: s.performance || '', operator: s.operator || s.localoperator || '', last_value: s.last_value || s.lastvalue || null, last_time: s.last_time || s.lasttime || '' };
  };
  ...
  const mergeWithPeru = (stations) => {
    ...
    const isPeruStation = (s) => {
      ...
      if (s.country === 'PER' || s.country === 'Peru') return PERU_STATIONS.some(p => Math.abs(p.lat - s.lat) < 0.15 && Math.abs(p.lon - s.lon) < 0.15);
      return false;
    };
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const pacific = allStations.map(mapStationFields).filter(s => s.code && s.lat && s.lon && isInPacific(s.lat, s.lon));
  ...
  useEffect(() => {
    ...
    filtered.forEach(s => {
      ...
      const icon = L.divIcon({ className: '', html: `<div style="display:flex;align-items:center;gap:4px;white-space:nowrap;"><div style="width:${isSel?14:10}px;height:${isSel?14:10}px;flex-shrink:0;background:${color};border:2px solid ${isSel?'#fbbf24':'#fff'};border-radius:${isOnline?'50%':'2px'};box-shadow:0 0 ${isSel?12:6}px ${color};"></div><span style="font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:${isSel?700:400};color:${isSel?'#fbbf24':'#cbd5e1'};background:rgba(5,11,24,0.78);padding:1px 3px;border-radius:2px;pointer-events:none;">${s.name}</span></div>`, iconSize: [10, 14], iconAnchor: [5, 7] });
      const marker = L.marker([s.lat, s.lon], { icon }).addTo(map);
      marker.bindTooltip(`<div style="font-family:monospace;font-size:11px;line-height:1.4"><b style="color:#fbbf24">${s.name}</b><br/><span style="color:#94a3b8">${s.country} | <b>${s.code}</b></span><br/><span style="color:${color}">${isOnline?'● ONLINE':'■ OFFLINE'}</span><span style="color:#64748b"> | ${s.lat?.toFixed(2)}, ${s.lon?.toFixed(2)}</span></div>`, { className: 'cnat-tooltip', direction: 'top', offset: [0, -8] });
  ...
  const handleStationClick = useCallback(async (station) => {
    ...
    try {
      ...
      if (d.data && d.data.length > 0) {
        ...
        const processed = d.data.map(p => ({ ...p, value: parseFloat((p.value-median).toFixed(4)), time: new Date(p.timestamp).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}) }));
        setTideData(processed);
        const relValues = processed.map(p => p.value);
        setStats({ min: Math.min(...relValues).toFixed(3), max: Math.max(...relValues).toFixed(3), mean: (relValues.reduce((a,b)=>a+b,0)/relValues.length).toFixed(3), range: (Math.max(...relValues)-Math.min(...relValues)).toFixed(3), points: relValues.length, median_abs: median.toFixed(3) });
      } else {
        try {
          const now = new Date(), start = new Date(now.getTime()-hours*3600000);
          const r2 = await fetch(`https://www.ioc-sealevelmonitoring.org/service.php?query=data&code=${station.code}&timestart=${start.toISOString().slice(0,16)}&timeend=${now.toISOString().slice(0,16)}&format=json`);
          const rawData = await r2.json();
          if (Array.isArray(rawData) && rawData.length > 0) {
            const processed = rawData.filter(p=>p.slevel!=null&&p.slevel!=='').map(p=>({ timestamp:p.stime||'', value:parseFloat(p.slevel), time:new Date(p.stime).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}) }));
            setTideData(processed);
            if (processed.length>0) { const values=processed.map(p=>p.value); setStats({ min:Math.min(...values).toFixed(3), max:Math.max(...values).toFixed(3), mean:(values.reduce((a,b)=>a+b,0)/values.length).toFixed(3), range:(Math.max(...values)-Math.min(...values)).toFixed(3), points:values.length }); }
          }
        } catch(corsErr) { console.warn('[CNAT] IOC v1 fallo CORS:', corsErr.message); }
      }
    } catch (e) { console.error('[CNAT] Error datos marea:', e); }
    setLoadingTide(false);
  }, [hours]);

  const onlineCount = stations.filter(s=>s.status==='online').length, offlineCount = stations.length - onlineCount;
  return (
    <div style={{ display: 'flex', height: '100%', gap: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #1e3a5f', background: '#0d1a2e' }}>
          <div><div style={{ fontSize: 12, fontWeight: 700, color: '#fbbf24', letterSpacing: 1 }}>MAPA MAREOGRAFICO DEL PACIFICO</div><div style={{ fontSize: 9, color: '#64748b', marginTop: 2 }}>Fuente: UNESCO/IOC Sea Level Station Monitoring Facility | Mapa: CartoDB Dark</div></div>
          <div style={{ display: 'flex', gap: 14, fontSize: 11 }}><span style={{ color: '#22c55e' }}>● ONLINE: <b>{onlineCount}</b></span><span style={{ color: '#ef4444' }}>■ OFFLINE: <b>{offlineCount}</b></span><span style={{ color: '#94a3b8' }}>TOTAL: <b>{stations.length}</b></span>{debugInfo && <span style={{ color: '#8b5cf6', fontSize: 9 }}>({debugInfo})</span>}</div>
        </div>
        <div style={{ display: 'flex', gap: 6, padding: '6px 16px', borderBottom: '1px solid #1e3a5f22', background: '#070e1f', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 6 }}>
            {['all','online','offline'].map(f=><button key={f} onClick={()=>setFilter(f)} style={{ background:filter===f?'#3b82f622':'transparent', border:`1px solid ${filter===f?'#3b82f6':'#1e3a5f'}`, color:filter===f?'#3b82f6':'#64748b', padding:'3px 12px', borderRadius:4, cursor:'pointer', fontSize:10, fontFamily:'inherit', letterSpacing:1 }}>{f==='all'?`TODAS (${stations.length})`:f==='online'?`ONLINE (${onlineCount})`:`OFFLINE (${offlineCount})`}</button>)}
          </div>
          <div style={{ display: 'flex', gap: 4 }}>
            {[{name:'PACIFICO',lat:0,lon:-150,zoom:3},{name:'PERU',lat:-10,lon:-77,zoom:6},{name:'CHILE',lat:-33,lon:-72,zoom:5},{name:'JAPON',lat:36,lon:140,zoom:5}].map(v=><button key={v.name} onClick={()=>mapInstanceRef.current?.flyTo([v.lat,v.lon],v.zoom,{duration:1.5})} style={{ background:'transparent', border:'1px solid #1e3a5f', color:'#fbbf24', padding:'3px 10px', borderRadius:4, cursor:'pointer', fontSize:9, fontFamily:'inherit', letterSpacing:1, fontWeight:700 }}>{v.name}</button>)}
          </div>
        </div>
        ...
        <div style={{ maxHeight: 140, overflow: 'auto', borderTop: '1px solid #1e3a5f', background: '#0d1a2e' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11 }}>
            <thead><tr style={{ color:'#64748b', textAlign:'left', position:'sticky', top:0, background:'#0d1a2e' }}><th style={{padding:'5px 10px'}}>STATUS</th><th style={{padding:'5px 8px'}}>CODIGO</th><th style={{padding:'5px 8px'}}>ESTACION</th><th style={{padding:'5px 8px'}}>PAIS</th><th style={{padding:'5px 8px'}}>LAT</th><th style={{padding:'5px 8px'}}>LON</th></tr></thead>
            <tbody>{stations.filter(s=>filter==='all'||s.status===filter).slice(0,50).map((s,idx)=><tr key={`${s.code}-${idx}`} onClick={()=>handleStationClick(s)} style={{ cursor:'pointer', background:selectedStation?.code===s.code?'#3b82f622':'transparent', borderBottom:'1px solid #1e3a5f22' }}><td style={{padding:'4px 10px'}}><span style={{display:'inline-block',width:8,height:8,borderRadius:s.status==='online'?'50%':2,background:s.status==='online'?'#22c55e':'#ef4444'}}/></td><td style={{padding:'4px 8px',color:'#3b82f6',fontWeight:700}}>{s.code}</td><td style={{padding:'4px 8px',color:'#e2e8f0'}}>{s.name}</td><td style={{padding:'4px 8px',color:'#64748b'}}>{s.country}</td><td style={{padding:'4px 8px',color:'#64748b'}}>{s.lat?.toFixed(2)}</td><td style={{padding:'4px 8px',color:'#64748b'}}>{s.lon?.toFixed(2)}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
      {selectedStation && (
        <div style={{ width:380, borderLeft:'1px solid #1e3a5f', background:'#0d1a2e', display:'flex', flexDirection:'column', overflow:'auto' }}>
          <div style={{ padding:'12px 14px', borderBottom:'1px solid #1e3a5f', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div><div style={{fontSize:9,color:'#64748b',letterSpacing:1}}>ESTACION SELECCIONADA</div><div style={{fontSize:16,fontWeight:700,color:'#fbbf24',marginTop:4}}>{selectedStation.name}</div><div style={{fontSize:11,color:'#94a3b8',marginTop:2}}>{selectedStation.country} • Codigo: <span style={{color:'#3b82f6'}}>{selectedStation.code||'⚠ SIN CODIGO'}</span></div></div>
          ...
          <div style={{display:'flex',gap:5,padding:'6px 14px',borderBottom:'1px solid #1e3a5f'}}>
            ...
            {[6,12,24,48].map(h=><button key={h} onClick={()=>{setHours(h);handleStationClick(selectedStation);}} style={{background:hours===h?'#06b6d422':'transparent',border:`1px solid ${hours===h?'#06b6d4':'#1e3a5f'}`,color:hours===h?'#06b6d4':'#64748b',padding:'2px 10px',borderRadius:4,cursor:'pointer',fontSize:11,fontFamily:'inherit'}}>{h}h</button>)}
          </div>
          <div style={{padding:'12px 8px 8px 0',height:260}}>
            ...
            {loadingTide?<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:200,color:'#64748b'}}><div style={{textAlign:'center'}}><div style={{fontSize:24,marginBottom:8}}>~</div>Cargando datos...</div></div>
            :tideData.length>0?<ResponsiveContainer width="100%" height={200}><LineChart data={tideData}><CartesianGrid strokeDasharray="3 3" stroke="#1e3a5f33" vertical={false}/><XAxis dataKey="time" tick={{fill:'#64748b',fontSize:9}} axisLine={{stroke:'#1e3a5f'}} tickLine={false} interval={Math.floor(tideData.length/8)}/><YAxis tick={{fill:'#64748b',fontSize:9}} axisLine={{stroke:'#1e3a5f'}} tickLine={false} domain={['auto','auto']} tickFormatter={v=>`${v}m`}/><Tooltip contentStyle={{background:'#0d1a2e',border:'1px solid #1e3a5f',borderRadius:6,fontSize:11,color:'#e2e8f0'}} formatter={value=>[`${value} m`,'Nivel del mar']} labelFormatter={label=>`Hora: ${label}`}/><ReferenceLine y={0} stroke="#64748b" strokeDasharray="5 5"/><Line type="monotone" dataKey="value" stroke="#06b6d4" strokeWidth={2} dot={false} activeDot={{r:4,fill:'#06b6d4',stroke:'#060B18',strokeWidth:2}}/></LineChart></ResponsiveContainer>
            :<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:200,color:'#64748b',fontSize:12,flexDirection:'column',gap:8}}>{!selectedStation.code?<><span style={{color:'#ef4444',fontSize:14}}>⚠</span><span>Estacion sin codigo IOC</span></>:<><span>No hay datos disponibles</span><span style={{fontSize:10,color:'#475569'}}>Codigo: {selectedStation.code} | Periodo: {hours}h</span></>}</div>}
          </div>
          {stats&&stats.points>0&&<div style={{padding:'10px 14px',borderTop:'1px solid #1e3a5f'}}><div style={{fontSize:9,color:'#64748b',letterSpacing:1,marginBottom:8}}>RESUMEN ESTADISTICO</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>{[{l:'Maximo',v:`${stats.max} m`,c:'#ef4444'},{l:'Minimo',v:`${stats.min} m`,c:'#3b82f6'},{l:'Promedio',v:`${stats.mean} m`,c:'#06b6d4'},{l:'Rango',v:`${stats.range} m`,c:'#f59e0b'}].map(item=><div key={item.l} style={{background:'#070e1f',border:'1px solid #1e3a5f44',borderRadius:6,padding:'6px 10px'}}><div style={{fontSize:9,color:'#64748b'}}>{item.l}</div><div style={{fontSize:15,fontWeight:700,color:item.c,marginTop:1,fontFamily:"'Orbitron'"}}>{item.v}</div></div>)}</div><div style={{fontSize:9,color:'#475569',marginTop:6,textAlign:'center'}}>{stats.points} puntos • Ultimas {hours} horas</div></div>}
          <div style={{padding:'10px 14px',borderTop:'1px solid #1e3a5f',fontSize:11}}><div style={{fontSize:9,color:'#64748b',letterSpacing:1,marginBottom:6}}>METADATA</div>{[['Fuente','IOC/SLSMF (UNESCO)'],['Sensor',selectedStation.sensor_type||'prs'],['Performance',selectedStation.performance||'N/A'],['Operador',selectedStation.operator||'N/A']].map(([k,v])=><div key={k} style={{display:'flex',justifyContent:'space-between',padding:'2px 0'}}><span style={{color:'#64748b'}}>{k}</span><span style={{color:'#e2e8f0',maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textAlign:'right'}}>{v}</span></div>)}</div>
        </div>
      )}
    </div>
  );
...
function AriaAssistant({ data }) {
  ...
  const prompts = [
    ...
    "Hay riesgo de tsunami para Peru?",
  ...
  const send = async (t) => {
    if (!t.trim()) return;
    const um = { role: 'user', content: t }, nm = [...messages, um];
    setMessages(nm); setInput(''); setLoading(true);
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': CLAUDE_KEY, 'anthropic-version': '2023-06-01', 'anthropic-dangerous-direct-browser-access': 'true' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, system: buildCtx(), messages: nm.map(m => ({ role: m.role, content: m.content })) }) });
      const d = await r.json();
      setMessages(p => [...p, { role: 'assistant', content: d.content?.[0]?.text || JSON.stringify(d) }]);
    } catch (e) { setMessages(p => [...p, { role: 'assistant', content: 'Error: ' + e.message }]); }
    setLoading(false);
  };

  const exportPDF = () => {
    const k = data?.kpis || {}; const now = new Date().toLocaleString('es-PE');
    const content = messages.map(m => `<div style="margin-bottom:20px;padding:14px;border-left:4px solid ${m.role === 'user' ? '#f59e0b' : '#8b5cf6'};background:${m.role === 'user' ? '#fff8e7' : '#f8f6ff'}"><div style="font-size:11px;font-weight:700;color:${m.role === 'user' ? '#92400e' : '#6d28d9'};margin-bottom:8px;letter-spacing:1px">${m.role === 'user' ? 'OPERADOR' : 'ARIA — Asistente IA CNAT'}</div><div style="font-size:13px;color:#1e293b;line-height:1.8;white-space:pre-wrap">${m.content}</div></div>`).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Reporte ARIA - CNAT ${now}</title><style>body{font-family:'Courier New',monospace;margin:0;padding:0;color:#1e293b}@media print{.no-print{display:none}}</style></head><body><div style="background:#0a1628;padding:24px 32px;color:#fff"><div style="display:flex;align-items:center;gap:16px"><div style="font-size:28px;font-weight:900;color:#f59e0b;letter-spacing:4px">CNAT</div><div><div style="font-size:11px;color:#94a3b8;letter-spacing:2px">CENTRO NACIONAL DE ALERTA DE TSUNAMIS</div><div style="font-size:9px;color:#475569;margin-top:2px">DIRECCIÓN DE HIDROGRAFÍA Y NAVEGACIÓN — MGP</div></div><div style="margin-left:auto;text-align:right"><div style="font-size:10px;color:#fbbf24">REPORTE ARIA — IA</div><div style="font-size:9px;color:#64748b">${now}</div></div></div></div><div style="background:#f1f5f9;padding:16px 32px;border-bottom:2px solid #e2e8f0;display:flex;gap:32px"><div style="font-size:11px"><span style="color:#64748b">SISMOS:</span> <b>${k.total_earthquakes||0}</b></div><div style="font-size:11px"><span style="color:#64748b">ALERTAS:</span> <b>${k.active_alerts||0}</b></div><div style="font-size:11px"><span style="color:#64748b">NIVEL DE RIESGO:</span> <b style="color:${k.risk_level==='ALTO'?'#dc2626':k.risk_level==='MEDIO'?'#d97706':'#16a34a'}">${k.risk_level||'BAJO'}</b></div><div style="font-size:11px"><span style="color:#64748b">BOYAS ACTIVAS:</span> <b>${k.alert_buoys||0}/${k.total_buoys||0}</b></div></div><div style="padding:24px 32px">${content||'<p style="color:#94a3b8;font-style:italic">Sin consultas registradas.</p>'}</div><div style="border-top:1px solid #e2e8f0;padding:12px 32px;display:flex;justify-content:space-between;font-size:9px;color:#94a3b8"><span>CNAT v2.0 — MICROHELP © 2026</span><span>Documento generado automáticamente por ARIA</span><span>CONFIDENCIAL — USO INTERNO</span></div></body></html>`;
      ...
      <div style={{ padding: 16, borderBottom: '2px solid #8b5cf6', background: '#0a1628' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: '#a78bfa' }}>Asistente IA - CNAT</div>
          </div>
          {messages.length > 0 && (
            <>
              <button onClick={exportPDF} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #6366f1', background: '#1e1b4b', color: '#a5b4fc', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, marginRight: 8 }}>⬇ PDF</button>
              <button onClick={() => setMessages([])} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #334155', background: '#0d1a2e', color: '#94a3b8', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>↩ Nueva consulta</button>
            </>
          )}
        </div>
      </div>
      ...
      {messages.length === 0 && (
        <div style={{ padding: 16 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700, letterSpacing: 1.5, marginBottom: 10 }}>
              ¿QUÉ ES ARIA?
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                ...
                <div style={{ fontSize: 12, color: '#e2e8f0', lineHeight: 1.6 }}>
                  <span style={{ color: '#fbbf24', fontWeight: 700 }}>Mira ARIA</span> para saber{' '}
                  <span style={{ color: '#a78bfa', fontWeight: 700 }}>qué está pasando ahora</span> — genera boletines con el estado actual del sistema en tiempo real.
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <span style={{ fontSize: 16, lineHeight: 1 }}>🔭</span>
                <div style={{ fontSize: 12, color: '#94a3b8', lineHeight: 1.6 }}>
                  Para buscar sismos pasados por región, fecha o magnitud, usa el módulo{' '}
                  <span style={{ color: '#06b6d4', fontWeight: 700 }}>VIGÍA (IA)</span>.
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 14 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ marginBottom: 14, padding: 14, borderRadius: 8, background: m.role === 'user' ? '#1e3a5f22' : '#0d1a2e', borderLeft: m.role === 'user' ? '4px solid #f59e0b' : '4px solid #8b5cf6' }}>
            <div style={{ fontSize: 11, color: m.role === 'user' ? '#fbbf24' : '#a78bfa', fontWeight: 700, marginBottom: 8 }}>{m.role === 'user' ? 'OPERADOR' : 'ARIA'}</div>
            <div style={{ fontSize: 13, color: '#e2e8f0', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>{m.content}</div>
          </div>
        ))}
        {loading && (
          <div style={{ padding: 14, borderRadius: 8, background: '#0d1a2e', borderLeft: '4px solid #8b5cf6' }}>
    ...
    <div style={{ minHeight:'100vh', background:'#050b18', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'JetBrains Mono', monospace" }}>
      ...
      <div style={{ width:420, background:'#0a1628', border:'1px solid #1e3a5f', borderRadius:12, overflow:'hidden', boxShadow:'0 0 60px rgba(59,130,246,0.1)' }}>
        ...
        <form onSubmit={handleLogin} style={{ padding:'32px' }}>
          ...
          <button type="submit" disabled={loading} style={{ width:'100%', padding:'14px', background:loading?'#334155':'linear-gradient(90deg,#1e40af,#3b82f6)', border:'none', borderRadius:6, color:'#fff', fontSize:13, fontWeight:700, letterSpacing:2, cursor:loading?'not-allowed':'pointer', fontFamily:'inherit' }}>{loading?'VERIFICANDO...':'INGRESAR AL SISTEMA'}</button>
...
function useAuth() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, s) => { setSession(s); if (s?.user) fetchProfile(s.user.id); else setUserProfile(null); });
...
function UsersTab() {
  ...
  useEffect(() => { supabase.from('cnat_users').select('*').order('created_at').then(({ data }) => { setUsers(data||[]); setLoading(false); }); }, []);
    ...
    <div>
      ...
      <div style={{ borderRadius:10, overflow:'hidden', border:'1px solid #1e3a5f' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', background:'#0d1a2e' }}>
          ...
          <tbody>{users.map(u=><tr key={u.id} style={{ borderBottom:'1px solid #1e3a5f22' }}><td style={{ padding:'10px 14px', fontSize:13, color:'#e2e8f0', fontWeight:600 }}>{u.full_name}</td><td style={{ padding:'10px 14px', fontSize:11, color:'#94a3b8' }}>{u.email}</td><td style={{ padding:'10px 14px' }}>{editingId===u.id?<select defaultValue={u.role} onChange={e=>updateRole(u.id,e.target.value)} style={{ background:'#0a1628', border:'1px solid #1e3a5f', color:'#e2e8f0', padding:'4px 8px', borderRadius:4, fontSize:11, fontFamily:'inherit' }}>{ROLES.map(r=><option key={r} value={r}>{r}</option>)}</select>:<span style={{ padding:'3px 10px', borderRadius:4, background:`${roleColor(u.role)}20`, color:roleColor(u.role), fontSize:11, fontWeight:700 }}>{u.role}</span>}</td><td style={{ padding:'10px 14px', fontSize:10, color:'#64748b' }}>{u.last_login?new Date(u.last_login).toLocaleString('es-PE'):'Nunca'}</td><td style={{ padding:'10px 14px' }}><span style={{ color:u.active?'#22c55e':'#ef4444', fontSize:11, fontWeight:700 }}>{u.active?'ACTIVO':'INACTIVO'}</span></td><td style={{ padding:'10px 14px', display:'flex', gap:6 }}><button onClick={()=>setEditingId(editingId===u.id?null:u.id)} style={{ padding:'4px 10px', background:'#1e3a5f', border:'none', borderRadius:4, color:'#fbbf24', fontSize:10, cursor:'pointer', fontFamily:'inherit' }}>ROL</button><button onClick={()=>toggleActive(u.id,u.active)} style={{ padding:'4px 10px', background:u.active?'#7f1d1d':'#14532d', border:'none', borderRadius:4, color:u.active?'#fca5a5':'#86efac', fontSize:10, cursor:'pointer', fontFamily:'inherit' }}>{u.active?'DESACTIVAR':'ACTIVAR'}</button></td></tr>)}</tbody>
        </table>
      </div>
...
export default function App() {
  ...
  const playAlert5s = useCallback(() => {
    try {
      ...
      const beep = (t) => { const o = ctx.createOscillator(), g = ctx.createGain(); o.connect(g); g.connect(ctx.destination); o.type = 'sine'; o.frequency.setValueAtTime(1400, t); o.frequency.exponentialRampToValueAtTime(900, t + 0.2); g.gain.setValueAtTime(0.5, t); g.gain.exponentialRampToValueAtTime(0.001, t + 0.4); o.start(t); o.stop(t + 0.4); };
  ...
  const clearFocus = useCallback(() => { setFocusedEqId(null); if (focusTimer.current) clearTimeout(focusTimer.current); }, []);
          ...
          {tab === 'boyas' && (
            <div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10 }}>
                {bu.map(b => { const c = b.status==='alert'?'#ef4444':b.status==='warning'?'#f59e0b':'#22c55e'; return <div key={b.id} style={{ background:'#0d1a2e', border:`1px solid ${c}33`, borderRadius:8, padding:14 }}><div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}><span style={{ fontSize:13, fontWeight:600, color:'#fbbf24' }}>{b.name}</span><div style={{ width:12, height:12, borderRadius:'50%', background:c }} /></div><div style={{ fontSize:10, color:'#94a3b8' }}>{b.country} | {b.latitude?.toFixed(2)},{b.longitude?.toFixed(2)}</div><div style={{ marginTop:6, textAlign:'center', padding:4, borderRadius:4, background:`${c}15` }}><span style={{ fontSize:11, fontWeight:700, color:c }}>{b.status==='normal'?'NORMAL':'ANOMALIA'}</span></div></div>; })}
              </div>
            </div>
          )}
