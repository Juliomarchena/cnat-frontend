...
export default function App() {
  ...
  const playPing = useCallback(() => { try { if(!pingRef.current) pingRef.current=new(window.AudioContext||window.webkitAudioContext)(); const ctx=pingRef.current,o=ctx.createOscillator(),g=ctx.createGain(); o.connect(g);g.connect(ctx.destination); o.type='sine';o.frequency.setValueAtTime(1200,ctx.currentTime);o.frequency.exponentialRampToValueAtTime(800,ctx.currentTime+0.15); g.gain.setValueAtTime(0.2,ctx.currentTime);g.gain.exponentialRampToValueAtTime(0.001,ctx.currentTime+0.3); o.start();o.stop(ctx.currentTime+0.3); }catch(e){} }, []);
  const focusEq = useCallback((id) => { setFocusedEqId(id); setTab('mapa'); playPing(); }, [playPing]);
  const clearFocus = useCallback(() => setFocusedEqId(null), []);
